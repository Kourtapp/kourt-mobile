...
const HeartIcon = ({ size = 18, color = '#9CA3AF' }: { size?: number; color?: string }) => (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
const CommentIcon = ({ size = 18, color = '#9CA3AF' }: { size?: number; color?: string }) => (
    <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
...
export function InviteCard({ invite, onJoin, onLike, onComment }: InviteCardProps) {
    ...
    return (
        ...
        >
                ...
                {invite.participants.map((p, i) => (
                    ...
                    >
                        {p.avatar ? (
                            ...
                            <PersonIcon size={18} color="#9CA3AF" />
                        )}
                    </View>
                ))}
                {emptySlots > 0 && (
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                {/* Likes */}
                <Pressable onPress={onLike} style={{ flexDirection: 'row', alignItems: 'center', marginRight: 16 }}>
                    <HeartIcon size={18} />
                    <Text style={{ fontSize: 14, color: '#6B7280', marginLeft: 6 }}>{invite.likes}</Text>
                </Pressable>

                {/* Comments */}
                <Pressable onPress={onComment} style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <CommentIcon size={18} />
