...
export default function RegisterStep4() {
    ...
    const [effort, setEffort] = useState(4); // 1-5
    const [mood, setMood] = useState<'sad' | 'neutral' | 'happy' | 'star'>('happy');
    const [aces, setAces] = useState(5);
    const [errors, setErrors] = useState(3);
    const [notes, setNotes] = useState('');

    const handlePublish = () => {
        // TODO: Submit data to backend
        // Navigate to Analysis/Success screen
        router.push('/post-match/analysis');
    };
    ...
    return (
        <View className="flex-1 bg-white">
            ...
            <View className="absolute bottom-0 left-0 right-0 p-5 pb-8 bg-white border-t border-gray-100">
                <Pressable
                    className="w-full bg-[#84cc16] h-14 rounded-full items-center justify-center active:opacity-90 transition-opacity"
